// MB Product Card - price04
//
// Family:
// Price
//
// Purpose:
// A price-family variant that presents the deal more like a tagged retail offer.
// This card is useful when the section wants the value story to feel curated,
// almost like a coupon or offer-ticket, without turning into a promo-family card.
//
// Footprint:
// Half-width card.
...
//
// Visual Priority:
// 1. Offer tag / coupon cue
// 2. Final price
// 3. Product title
// 4. Old price / savings
// 5. Small retail support text
//
// Best Use Cases:
// Offer-led categories, basket builders, coupon-style pricing moments,
// grocery combo sections, and value-focused curated lists.
//
// Behavior Notes:
// - Keeps the price family footprint.
// - Uses a strong offer-tag header style.
// - Feels more merchandised than price01/02/03.
// - Still remains product-first, not campaign-first.
...
class MBProductCardPrice04 extends StatelessWidget {
  const MBProductCardPrice04({
    super.key,
  ...
  Widget build(BuildContext context) {
    final offerTag = _buildOfferTag(context);
  ...
  Widget? _buildOfferTag(BuildContext context) {
    final text = _offerTagText();
  ...
  Widget? _buildSavingsLine(BuildContext context) {
    ...
    return Text(
      'Save ৳${saved.toStringAsFixed(0)} with this offer',
      maxLines: 1,
      overflow: TextOverflow.ellipsis,
      style: Theme.of(context).textTheme.labelMedium?.copyWith(
            color: const Color(0xFFE67E22),
