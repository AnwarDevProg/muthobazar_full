// MB Product Card - compact01
//
// Family:
// Compact
//
// Purpose:
// A fast-scanning everyday product card for dense 2-column store grids.
// This is the default lightweight shopping card for categories where users
// need to see many products quickly without too much visual noise.
//
// Footprint:
// Half-width card.
...
// 2. Product title
// 3. Current price
// 4. Old price / discount badge
// 5. Small supporting metadata
//
// Best Use Cases:
// Grocery, personal care, baby items, household essentials, repeat-purchase products.
//
// Behavior Notes:
// - Title clamps to 2 lines.
// - Image feels prominent but not oversized.
// - Badge remains small and non-dominant.
// - Price area stays readable at a glance.
// - Works safely in store preview grids with fixed/bounded height.
// - Degrades gracefully when sale price, badge, or metadata are missing.
...
class MBProductCardCompact01 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final metaRow = _buildBottomMeta(context);
  ...
  Widget _buildPriceBlock(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        if (resolved.price.showsDiscountInfo && hasDiscount) ...<Widget>[
          const SizedBox(height: 4),
          Text(
  ...
  Widget? _buildBottomMeta(BuildContext context) {
    ...
    if (resolved.meta.showStockHint && stockQty > 0 && stockQty <= 10) {
      texts.add('Low stock');
    }
