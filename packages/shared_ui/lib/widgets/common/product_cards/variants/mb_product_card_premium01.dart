// MB Product Card - premium01
//
// Family:
// Premium
//
// Purpose:
// A polished half-width product card for branded, higher-value, or more
// presentation-sensitive products where visual refinement matters more than
// raw density or aggressive pricing.
//
// Footprint:
// Half-width card.
// Medium to slightly tall card.
// Designed for 2 cards per row while still feeling more elevated than compact
// and price-focused cards.
//
// Visual Priority:
// 1. Product image
// 2. Product title
...
//
// Best Use Cases:
// Electronics, beauty, branded household items, premium grocery, showcase tiles.
//
// Behavior Notes:
// - Layout should feel cleaner and calmer than price-focused cards.
// - Badge usage must remain restrained.
// - Title should clamp to 2 lines.
// - Brand or premium cue may appear, but must not overpower the product.
// - Must work safely in bounded preview containers and scrollable layouts.
// - Must remain elegant even when sale fields are absent.
...
class MBProductCardPremium01 extends StatelessWidget {
  const MBProductCardPremium01({
    super.key,
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  if (resolved.actions.showViewDetails ||
                      resolved.actions.showAddToCart) ...<Widget>[
                    const SizedBox(height: 12),
  ...
  EdgeInsets _contentPadding() {
    ...
    return EdgeInsets.all(base.clamp(10.0, 22.0));
  }
  ...
  Widget? _buildBrandChip(BuildContext context) {
    ...
    final brandName = _textValue(product.brandNameEn);
