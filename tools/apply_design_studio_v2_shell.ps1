# MuthoBazar - Apply Design Studio V2 Shell Patch
# -----------------------------------------------
# Creates the V2 design studio shell beside the current working editor.
#
# Adds:
# packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/...
#
# Also appends shared_ui.dart export:
...
# powershell -ExecutionPolicy Bypass -File .\tools\apply_design_studio_v2_shell.ps1
# flutter analyze
...
$TempRoot = Join-Path $env:TEMP "muthobazar_design_studio_v2_shell_$Timestamp"
$BackupRoot = Join-Path (Split-Path $RepoRoot) "MuthoBazar_PatchBackups\design_studio_v2_shell_$Timestamp"

Write-Host ""
Write-Host "MuthoBazar Design Studio V2 Shell Patch" -ForegroundColor Cyan
Write-Host "Repo root: $RepoRoot"
Write-Host ""

function Find-PatchZip {
  $Downloads = Join-Path $env:USERPROFILE "Downloads"
  $Candidate = Get-ChildItem -LiteralPath $Downloads -Filter "muthobazar_design_studio_v2_shell_patch*.zip" -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1

  if ($null -eq $Candidate) {
    throw "Could not find muthobazar_design_studio_v2_shell_patch*.zip in Downloads."
  }
...
try {
  ...
  $Files = @(
    "packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/models/mb_design_node.dart",
    "packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/models/mb_design_node_variant.dart",
    "packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/panels/mb_design_element_drawer.dart",
    "packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/panels/mb_design_canvas_panel.dart",
    "packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/panels/mb_design_inspector_panel.dart",
    "packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/mb_card_design_studio_v2.dart",
    "packages/shared_ui/lib/widgets/common/product_cards/design_studio_v2/mb_card_design_studio_v2_exports.dart"
  )
  ...
  $SharedUiPath = Join-Path $RepoRoot "packages\shared_ui\lib\shared_ui.dart"
  if (Test-Path -LiteralPath $SharedUiPath) {
    ...
    if (!$SharedUiText.Contains($ExportLine)) {
      ...
      Write-Host "Added shared_ui export." -ForegroundColor Green
    } else {
      Write-Host "shared_ui export already exists." -ForegroundColor DarkGray
    }
  } else {
    Write-Host "shared_ui.dart not found, skipped export." -ForegroundColor Yellow
  }
  ...
  Write-Host 'Select-String -Path .\packages\shared_ui\lib\shared_ui.dart -Pattern "design_studio_v2"'
}
finally {
  Remove-Item -LiteralPath $TempRoot -Recurse -Force -ErrorAction SilentlyContinue
}
