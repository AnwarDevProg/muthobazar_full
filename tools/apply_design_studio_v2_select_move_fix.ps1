# MuthoBazar - Apply Design Studio V2 Select + Move Fix
# -----------------------------------------------------
# Fixes:
# - Click node selects node.
# - Mouse drag moves node.
...
# powershell -ExecutionPolicy Bypass -File .\tools\apply_design_studio_v2_select_move_fix.ps1
# flutter analyze
...
$TempRoot = Join-Path $env:TEMP "muthobazar_design_studio_v2_select_move_fix_$Timestamp"
$BackupRoot = Join-Path (Split-Path $RepoRoot) "MuthoBazar_PatchBackups\design_studio_v2_select_move_fix_$Timestamp"

Write-Host ""
Write-Host "MuthoBazar Design Studio V2 Select + Move Fix" -ForegroundColor Cyan
Write-Host "Repo root: $RepoRoot"
Write-Host ""

function Find-PatchZip {
  $Downloads = Join-Path $env:USERPROFILE "Downloads"
  $Candidate = Get-ChildItem -LiteralPath $Downloads -Filter "muthobazar_design_studio_v2_select_move_fix_patch*.zip" -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1

  if ($null -eq $Candidate) {
    throw "Could not find muthobazar_design_studio_v2_select_move_fix_patch*.zip in Downloads."
  }
...
try {
  ...
  Write-Host ""
  Write-Host "Verify:"
  Write-Host 'Select-String -Path .\packages\shared_ui\lib\widgets\common\product_cards\design_studio_v2\panels\mb_design_canvas_panel.dart -Pattern "Select/Move Fixed Canvas|direct Positioned|onPanUpdate|onTapDown"'
}
finally {
  Remove-Item -LiteralPath $TempRoot -Recurse -Force -ErrorAction SilentlyContinue
}
