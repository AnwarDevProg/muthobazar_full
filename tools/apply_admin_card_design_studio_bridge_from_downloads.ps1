# MuthoBazar - Apply Admin Card Design Studio Bridge Patch From Downloads
# ---------------------------------------------------------------------
# Adds a save-capable reusable MBCardDesignStudio and an admin dialog bridge.
#
# This phase does NOT write Firestore automatically.
# It installs:
# 1. Updated shared MBCardDesignStudio with initial JSON + onSave support.
# 2. AdminCardDesignStudioDialog that returns design JSON to caller.
#
# Run from repo root:
#
# cd C:\Users\1\AndroidStudioProjects\MuthoBazar
# powershell -ExecutionPolicy Bypass -File .\tools\apply_admin_card_design_studio_bridge_from_downloads.ps1
#
# Optional:
# powershell -ExecutionPolicy Bypass -File .\tools\apply_admin_card_design_studio_bridge_from_downloads.ps1 -ZipPath "C:\Users\1\Downloads\muthobazar_admin_card_design_studio_bridge_patch.zip"
...
Write-Host ""
Write-Host "MuthoBazar Admin Card Design Studio Bridge Patch" -ForegroundColor Cyan
...
function Resolve-PatchZip {
  ...
  $Candidate = Get-ChildItem -LiteralPath $Downloads -Filter "muthobazar_admin_card_design_studio_bridge_patch*.zip" -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1

  if ($null -eq $Candidate) {
    throw "Could not find muthobazar_admin_card_design_studio_bridge_patch*.zip in Downloads."
  }
...
$TempExtract = Join-Path $env:TEMP "muthobazar_admin_card_design_studio_bridge_$Timestamp"
$BackupRoot = Join-Path (Split-Path $RepoRoot) "MuthoBazar_PatchBackups\admin_card_design_studio_bridge_$Timestamp"
...
try {
  ...
  Add-ExportLine `
    -FilePath $SharedUiBarrel `
    -ExportLine "export 'widgets/common/product_cards/design_studio/mb_card_design_studio.dart';"
