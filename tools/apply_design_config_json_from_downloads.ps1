# MuthoBazar - Apply Design Config JSON Patch From Downloads
# ---------------------------------------------------------
# Adds Copy/Paste/View/Print/Reset design config JSON controls
# to the Chat design lab.
#
# Run from repo root:
#
# cd C:\Users\1\AndroidStudioProjects\MuthoBazar
# powershell -ExecutionPolicy Bypass -File .\tools\apply_design_config_json_from_downloads.ps1
#
# Optional:
# powershell -ExecutionPolicy Bypass -File .\tools\apply_design_config_json_from_downloads.ps1 -ZipPath "C:\Users\1\Downloads\muthobazar_design_config_json_patch.zip"
...
Write-Host ""
Write-Host "MuthoBazar Design Config JSON Patch" -ForegroundColor Cyan
...
function Resolve-PatchZip {
  ...
  $Candidate = Get-ChildItem -LiteralPath $Downloads -Filter "muthobazar_design_config_json_patch*.zip" -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1

  if ($null -eq $Candidate) {
    throw "Could not find muthobazar_design_config_json_patch*.zip in Downloads."
  }
...
$TempExtract = Join-Path $env:TEMP "muthobazar_design_config_json_$Timestamp"
$BackupRoot = Join-Path (Split-Path $RepoRoot) "MuthoBazar_PatchBackups\design_config_json_$Timestamp"
