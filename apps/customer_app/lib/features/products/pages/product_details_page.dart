...
class _ProductDetailsPageState extends State<ProductDetailsPage> {
  ...
  void _selectInitialVariation() {
    ...
    final defaultVariation = product.variations.cast<MBProductVariation?>().firstWhere(
          (item) => item != null && item.isDefault,
      orElse: () => null,
    );

    final firstEnabled = product.variations.cast<MBProductVariation?>().firstWhere(
          (item) => item != null && item.isEnabled,
      orElse: () => null,
    );

    final selected = defaultVariation ?? firstEnabled ?? product.variations.first;
  ...
  bool get _isPurchasable {
    ...
    if (_isVariableProduct(product)) {
      ...
      if (!variation.isEnabled) return false;
  ...
  String get _availabilityText {
    ...
    if (_isVariableProduct(product)) {
      ...
      if (!variation.isEnabled) {
        return 'Selected variation is disabled';
      }
