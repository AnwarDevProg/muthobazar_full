...
class _ProductVariationDialogState extends State<ProductVariationDialog> {
  ...
  bool get _defaultLockedByOtherVariation {
    final otherDefaultId = widget.currentDefaultVariationId?.trim();
    if (otherDefaultId == null || otherDefaultId.isEmpty) return false;
    return otherDefaultId != _idController.text.trim();
  }
  ...
  List<String> _selectedAttributeCombinationLines() {
    final lines = <String>[];
    for (final attribute in widget.variationAttributes) {
      final selected = (_selectedAttributeValues[attribute.id] ?? '').trim();
      if (selected.isEmpty) continue;
      final matched = attribute.values.cast<MBProductAttributeValue?>().firstWhere(
            (item) => item != null && item.value.trim() == selected,
            orElse: () => null,
          );
      final attributeName = attribute.nameEn.trim().isEmpty
          ? (attribute.code.trim().isEmpty ? attribute.id : attribute.code)
          : attribute.nameEn.trim();
      final valueName = matched == null
          ? selected
          : (matched.labelEn.trim().isEmpty ? matched.value : matched.labelEn.trim());
      lines.add('$attributeName: $valueName');
    }
    return lines;
  }
    ...
    return SectionCard(
      ...
      child: Column(
        ...
        children: [
          ...
          ...widget.variationAttributes.map((attribute) {
            ...
            children: [
              ...
              Expanded(
                child: Wrap(
                  ...
                  children: [
                    Tooltip(
                      message: _defaultLockedByOtherVariation
                          ? 'Another variation is already selected as default. Edit that variation or save with the existing default.'
                          : 'Set this variation as the product default.',
                      child: IgnorePointer(
                        ignoring: _defaultLockedByOtherVariation,
                        child: Opacity(
                          opacity: _defaultLockedByOtherVariation ? 0.45 : 1.0,
