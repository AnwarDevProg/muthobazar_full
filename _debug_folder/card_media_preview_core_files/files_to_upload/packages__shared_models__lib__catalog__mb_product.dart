...
class MBProduct {
  ...
  List<MBProductMedia> get enabledMediaItems {
    final items = mediaItems.where((item) => item.isEnabled).toList();
    items.sort((a, b) => a.sortOrder.compareTo(b.sortOrder));
    return items;
  }

  MBProductMedia? get primaryMediaItem {
    for (final item in enabledMediaItems) {
      if (item.isPrimary || item.role == 'thumbnail') {
        return item;
      }
    }

    final items = enabledMediaItems;
    return items.isEmpty ? null : items.first;
  }

  String _firstEnabledMediaUrl(String Function(MBProductMedia item) selector) {
    final primary = primaryMediaItem;
    if (primary != null) {
      final primaryUrl = selector(primary).trim();
      if (primaryUrl.isNotEmpty) return primaryUrl;
    }

    for (final item in enabledMediaItems) {
      if (primary != null && item.id == primary.id) continue;
      final url = selector(item).trim();
  ...
  List<String> get resolvedCardImageUrls {
    ...
    for (final item in enabledMediaItems) {
      if (item.type != 'image') continue;
      final url = item.effectiveCardUrl.trim();
      if (url.isNotEmpty && !urls.contains(url)) {
        urls.add(url);
      }
    }
