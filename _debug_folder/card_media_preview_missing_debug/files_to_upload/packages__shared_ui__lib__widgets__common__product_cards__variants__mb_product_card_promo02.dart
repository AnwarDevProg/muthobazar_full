// MB Product Card - promo02
//
// Family:
// Promo
//
// Purpose:
// A cleaner campaign-sensitive promo-family variant that keeps product clarity
// stronger than promo01 while still carrying a visible promotional identity.
//
// Footprint:
// Full-width card.
// Medium to tall footprint.
// Designed for campaign rows that should stay product-led.
//
// Visual Priority:
// 1. Promo strip
// 2. Product image
...
//
// Best Use Cases:
// Weekly campaigns, themed offers, curated landing rows, seasonal home-page
// promotions, and category-level campaign highlights.
//
// Behavior Notes:
// - Cleaner than promo01.
// - Keeps campaign styling visible but controlled.
// - Product remains the main focus.
// - Gracefully handles missing sale, subtitle, and promo signals.
...
class MBProductCardPromo02 extends StatelessWidget {
  const MBProductCardPromo02({
    super.key,
  ...
  Widget? _buildPromoStrip(BuildContext context) {
    ...
    if (!resolved.accent.showPromoStrip || text.isEmpty) {
      return null;
    }
  ...
  String _promoStripText() {
    if (product.isFlashSale) return 'Campaign flash promotion';
    if (product.isFeatured) return 'Curated campaign pick';
  ...
  String? _offerText() {
    ...
    if (salePrice != null && salePrice < product.price) {
      ...
      if (saved > 0) {
        return 'Save ৳${saved.toStringAsFixed(0)} on this promotion';
      }
    }
    if (product.isFlashSale) {
      return 'Limited-time campaign item';
    }
    return 'Special promotional highlight';
  }
