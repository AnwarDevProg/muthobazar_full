// MB Product Card - promo03
//
// Family:
// Promo
//
// Purpose:
// A stronger festive-ribbon promo-family variant intended for louder campaign
// moments where the promotional tone should feel warmer and more celebratory.
//
// Footprint:
// Full-width card.
// Medium to tall footprint.
// Designed for high-visibility campaign rows.
//
// Visual Priority:
// 1. Festive promo ribbon
// 2. Product image
// 3. Product title
// 4. Offer message
// 5. Price / CTA
//
// Best Use Cases:
// Eid campaigns, Ramadan rows, sale festivals, themed promotion pages,
// and louder curated offers.
//
// Behavior Notes:
// - More energetic than promo01 and promo02.
// - Still keeps the product clearly visible.
// - Uses warmer decoration and stronger campaign treatment.
// - Gracefully handles missing sale, subtitle, or campaign state.
...
class MBProductCardPromo03 extends StatelessWidget {
  const MBProductCardPromo03({
    super.key,
  ...
  Widget build(BuildContext context) {
    final ribbon = _buildRibbon(context);
    final badge = _buildPrimaryBadge(context);
    final subtitle = _buildSubtitle(context);
    final campaignBox = _buildCampaignBox(context);
  ...
  Widget? _buildCampaignBox(BuildContext context) {
    final text = _campaignText();
    if (text == null || text.isEmpty) return null;
  ...
  String _ribbonText() {
    if (product.isFlashSale) return 'Festival flash campaign';
    if (product.isFeatured) return 'Seasonal curated campaign';
    return 'Special festive promotion';
  }
  ...
  String? _campaignText() {
    ...
    if (salePrice != null && salePrice < product.price) {
      ...
      return 'Campaign saving: ৳${saved.toStringAsFixed(0)}';
    }
    if (product.isFlashSale) return 'Limited promotional window';
    return 'Celebrate this curated campaign pick';
  }

  String? _badgeText() {
    if (product.isFlashSale) return 'Flash';
    if (product.isFeatured) return 'Promo';
