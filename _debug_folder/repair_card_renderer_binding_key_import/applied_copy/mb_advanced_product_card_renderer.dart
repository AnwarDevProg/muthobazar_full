...
// --------------------------------------
// Patch 12.6: Runtime renderer support for full model binding context.
//
// Purpose:
...
class MBAdvancedProductCardRenderer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final previewContext = MBAdvancedPreviewContext.fromProduct(
      product: product,
      brand: brand,
      category: category,
...
class _RuntimeNodeContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    switch (node.elementType) {
      case 'media':
      case 'product_image':
        return _MediaNode(previewContext: previewContext, node: node);
      ...
      case 'secondarycta':
        return _CtaNode(
          label: _resolveNodeText(previewContext, node),
          node: node,
      ...
      case 'pricebadge':
      case 'price_badge':
        return _PriceNode(previewContext: previewContext, node: node);

      case 'mrp':
      case 'old_price':
      case 'original_price':
        return _MrpNode(previewContext: previewContext, node: node);
      ...
      case 'attribute_preset':
        return _BadgeOrTextNode(previewContext: previewContext, node: node);
      ...
      case 'progress':
      case 'stock_progress':
        return _ProgressNode(previewContext: previewContext, node: node);
...
class _BadgeOrTextNode extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final text = _resolveNodeText(previewContext, node);
...
class _ProgressNode extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final progress = _resolveProgress(previewContext, node).clamp(0.0, 1.0);
...
String _resolveNodeText(
  ...
  final fallback = node.readString(
    'fallbackText',
    fallback: MBAdvancedBindingResolver.resolveText(
      previewContext,
      binding,
      fallback: '',
    ),
  ...
  if (_isCurrencyBinding(binding)) {
    final number = MBAdvancedBindingResolver.resolveNumber(
      previewContext,
      binding,
    );
    if (number != null) {
      return _applyPrefixSuffix(_formatCurrency(number, node), node);
    }
  }

  final value = MBAdvancedBindingResolver.resolveText(
    previewContext,
    binding,
    fallback: fallback,
  );
...
bool _isCurrencyBinding(String binding) {
  return binding.toLowerCase().contains('price') ||
      binding.toLowerCase().contains('mrp') ||
      binding.toLowerCase().contains('cost');
}
...
Uint8List? _resolveImageBytes(
  ...
  final primary = product.primaryMediaItem;
  if (primary == null) return null;

  final normalized = binding.trim();

  switch (normalized) {
    case MBAdvancedBindingKey.productResolvedOriginalImageUrl:
      return primary.pendingOriginalBytes ??
          primary.pendingFullBytes ??
          primary.pendingCardBytes ??
          primary.pendingThumbBytes;
    case MBAdvancedBindingKey.productResolvedFullImageUrl:
      return primary.pendingFullBytes ??
          primary.pendingOriginalBytes ??
          primary.pendingCardBytes ??
          primary.pendingThumbBytes;
    case MBAdvancedBindingKey.productResolvedThumbImageUrl:
    case MBAdvancedBindingKey.productThumbnailUrl:
      return primary.pendingThumbBytes ??
          primary.pendingCardBytes ??
          primary.pendingFullBytes;
    case MBAdvancedBindingKey.productResolvedTinyImageUrl:
      return primary.pendingTinyBytes ??
          primary.pendingThumbBytes ??
          primary.pendingCardBytes ??
          primary.pendingFullBytes;
    ...
    default:
      return primary.pendingCardBytes ??
          primary.pendingFullBytes ??
          primary.pendingThumbBytes ??
          primary.pendingOriginalBytes;
  }
...
num _readFinalPrice(MBAdvancedPreviewContext previewContext) {
  return MBAdvancedBindingResolver.resolveNumber(
        previewContext,
        'product.finalPrice',
      ) ??
      0;
...
num _readOriginalPrice(MBAdvancedPreviewContext previewContext) {
  return MBAdvancedBindingResolver.resolveNumber(
        previewContext,
        'product.price',
      ) ??
      _readFinalPrice(previewContext);
}

bool _shouldStrikeOriginalPrice(
  MBAdvancedPreviewContext previewContext,
  _V3DesignNode node,
) {
  ...
  final original = _readOriginalPrice(previewContext).toDouble();
  final finalPrice = _readFinalPrice(previewContext).toDouble();
...
double _resolveProgress(
  ...
  final fromBinding = MBAdvancedBindingResolver.resolveNumber(
    previewContext,
    node.binding,
  );
  if (fromBinding != null && fromBinding > 0 && fromBinding <= 1) {
    return fromBinding.toDouble();
  }

  final stock = MBAdvancedBindingResolver.resolveNumber(
        previewContext,
        'variation.stockQty',
      ) ??
      MBAdvancedBindingResolver.resolveNumber(
        previewContext,
        'product.stockQty',
      ) ??
      0;
