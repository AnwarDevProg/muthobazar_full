...
class AdminProductController extends GetxController {
  ...
  void _validateBasicProductFields(
    ...
    if (product.reservedQty < 0) {
      blocking.add('Reserved quantity cannot be negative.');
    }
    ...
    if (product.status.trim().isEmpty) {
      blocking.add('Product status is required.');
    }

    if (product.vatRate != null && product.vatRate! < 0) {
      blocking.add('VAT rate cannot be negative.');
    }

    if (product.weightValue != null && product.weightValue! < 0) {
      blocking.add('Weight value cannot be negative.');
    }

    if (product.length != null && product.length! < 0) {
      blocking.add('Length cannot be negative.');
    }

    if (product.width != null && product.width! < 0) {
      blocking.add('Width cannot be negative.');
    }

    if (product.height != null && product.height! < 0) {
      blocking.add('Height cannot be negative.');
    }

    if (product.metadata.length > 50) {
      warnings.add('Product metadata has many keys. Keep metadata small and intentional.');
    }
  ...
  void _validateProductOperationalRules(
    ...
    if (product.trackInventory && product.regularStockQty < product.reservedQty) {
      warnings.add(
        'Reserved quantity is greater than regular stock quantity.',
      );
    }
  ...
  void _validateVariations(
    ...
    for (var index = 0; index < variations.length; index++) {
      ...
      if (variation.estimatedSchedulePrice != null &&
          variation.estimatedSchedulePrice! < 0) {
        blocking.add('$label estimated schedule price cannot be negative.');
      }
      ...
      if (variation.todayInstantCap < 0) {
        blocking.add('$label today instant cap cannot be negative.');
      }

      if (variation.todayInstantSold < 0) {
        blocking.add('$label today instant sold cannot be negative.');
      }

      if (variation.todayInstantSold > variation.todayInstantCap) {
        blocking.add('$label today instant sold cannot be greater than cap.');
      }

      if (variation.maxScheduleQtyPerDay < 0) {
        blocking.add('$label max schedule quantity cannot be negative.');
      }

      if (variation.minScheduleNoticeHours < 0) {
        blocking.add('$label minimum schedule notice hours cannot be negative.');
      }

      if (variation.reorderLevel < 0) {
        blocking.add('$label reorder level cannot be negative.');
      }

      if (variation.quantityValue < 0) {
        blocking.add('$label quantity value cannot be negative.');
      }

      if (variation.tolerance < 0) {
        blocking.add('$label tolerance cannot be negative.');
      }

      if (variation.minOrderQty != null && variation.minOrderQty! < 0) {
        blocking.add('$label minimum order quantity cannot be negative.');
      }

      if (variation.maxOrderQty != null && variation.maxOrderQty! < 0) {
        blocking.add('$label maximum order quantity cannot be negative.');
      }

      if (variation.maxOrderQty != null &&
          variation.minOrderQty != null &&
          variation.maxOrderQty! < variation.minOrderQty!) {
        blocking.add('$label maximum order quantity cannot be smaller than minimum order quantity.');
      }

      if (variation.stepQty != null && variation.stepQty! < 0) {
        blocking.add('$label step quantity cannot be negative.');
      }

      if (variation.status.trim().isEmpty) {
        blocking.add('$label status is required.');
      }

      if (variation.vatRate != null && variation.vatRate! < 0) {
        blocking.add('$label VAT rate cannot be negative.');
      }

      if (variation.weightValue != null && variation.weightValue! < 0) {
        blocking.add('$label weight value cannot be negative.');
      }

      if (variation.length != null && variation.length! < 0) {
        blocking.add('$label length cannot be negative.');
      }

      if (variation.width != null && variation.width! < 0) {
        blocking.add('$label width cannot be negative.');
      }

      if (variation.height != null && variation.height! < 0) {
        blocking.add('$label height cannot be negative.');
      }

      final variationSaleStartsAt = variation.saleStartsAt;
      final variationSaleEndsAt = variation.saleEndsAt;
      if (variationSaleStartsAt != null &&
          variationSaleEndsAt != null &&
          variationSaleStartsAt.isAfter(variationSaleEndsAt)) {
        blocking.add('$label sale start date must be before sale end date.');
      }

      final variationPublishAt = variation.publishAt;
      final variationUnpublishAt = variation.unpublishAt;
      if (variationPublishAt != null &&
          variationUnpublishAt != null &&
          variationPublishAt.isAfter(variationUnpublishAt)) {
        blocking.add('$label publish date must be before unpublish date.');
      }

      final variationCutoffTime = (variation.instantCutoffTime ?? '').trim();
      if (variationCutoffTime.isNotEmpty && !_isValidTime24h(variationCutoffTime)) {
        blocking.add('$label instant cutoff time must use HH:mm format.');
      }

      _validateVariationPurchaseOptions(
        variation: variation,
  ...
  void _validateVariationPurchaseOptions({
    required MBProductVariation variation,
    required String variationLabel,
    required List<String> blocking,
    required List<String> warnings,
  }) {
    final purchaseOptions = variation.purchaseOptions;
    ...
    for (var index = 0; index < purchaseOptions.length; index++) {
      ...
      final optionLabel = '$variationLabel purchase option ${index + 1}';

      final optionId = option.id.trim();
      if (optionId.isEmpty) {
        blocking.add('$optionLabel must have an id.');
      } else if (!seenOptionIds.add(_normalizedKey(optionId))) {
        blocking.add('$variationLabel has duplicate purchase option id: $optionId.');
      }

      if (option.labelEn.trim().isEmpty) {
        blocking.add('$optionLabel must have an English label.');
      }

      if (option.price < 0) {
        blocking.add('$optionLabel price cannot be negative.');
      }

      if (option.salePrice != null && option.salePrice! < 0) {
        blocking.add('$optionLabel sale price cannot be negative.');
      }

      if (option.salePrice != null && option.salePrice! >= option.price) {
        blocking.add('$optionLabel sale price must be smaller than regular price.');
      }

      if (option.minScheduleDays < 0 || option.maxScheduleDays < 0) {
        blocking.add('$optionLabel schedule days cannot be negative.');
      }

      if (option.maxScheduleDays < option.minScheduleDays) {
        blocking.add('$optionLabel maximum schedule days cannot be smaller than minimum schedule days.');
      }

      final cutoffTime = (option.cutoffTime ?? '').trim();
      if (cutoffTime.isNotEmpty && !_isValidTime24h(cutoffTime)) {
        blocking.add('$optionLabel cutoff time must use HH:mm format.');
      }

      if (option.isDefault) {
        defaultOptionCount++;
        if (!option.isEnabled) {
          blocking.add('$optionLabel is default but disabled.');
        }
      }
    }

    if (defaultOptionCount > 1) {
      warnings.add('$variationLabel has more than one default purchase option.');
    }
  }
