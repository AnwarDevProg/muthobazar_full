...
exports.checkPhoneAuthEligibility = (0, https_1.onCall)({
    ...
    try {
        ...
        // customer_app + register
        if (app === "customer_app" && intent === "register") {
            if (!existsInPhoneIndex) {
                response.authMode = "customer_register";
                response.allowSendOtp = true;
                response.code = "CUSTOMER_REGISTER_ALLOWED";
                response.message = "Phone number is eligible for customer registration.";
                return response;
            }
            response.authMode = "blocked";
            response.allowSendOtp = false;
            response.code = "CUSTOMER_REGISTER_ALREADY_EXISTS";
            response.message = "This number is already registered. Please login instead.";
            return response;
        }
        // admin_web + login
        if (app === "admin_web" && intent === "login") {
            if (bootstrapOpen && !superAdminExists) {
                ...
                response.code = "ADMIN_LOGIN_FIRST_SUPER_ADMIN_REQUIRED";
                response.message =
                    "No super admin exists yet. Continue to create the first super admin.";
                return response;
            }
            if (!existsInPhoneIndex) {
                response.authMode = "blocked";
            ...
            if ((userRole === "admin" || userRole === "super_admin") && canAccessAdminPanel) {
            ...
            response.message = "This number is not assigned to any active admin account.";
            return response;
        }
        // admin_web + register
        if (app === "admin_web" && intent === "register") {
            // IMPORTANT:
            // Existing phone must be blocked BEFORE bootstrap decision.
            if (existsInPhoneIndex) {
