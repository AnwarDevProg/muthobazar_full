...
function buildUpdatedCategoryMap(categoryId, payload, existing) {
    return {
        id: categoryId,
...
function buildAuditCategoryState(categoryId, payload, existingProductsCount) {
    return {
        id: categoryId,
        nameEn: payload.nameEn,
        nameBn: payload.nameBn,
...
function listChangedFields(existing, payload) {
    const changed = [];
    const nextGroupId = groupIdFromParentId(payload.parentId);
    if (existing.nameEn !== payload.nameEn)
        changed.push("nameEn");
    if (existing.nameBn !== payload.nameBn)
        changed.push("nameBn");
    if (existing.descriptionEn !== payload.descriptionEn)
        changed.push("descriptionEn");
    if (existing.descriptionBn !== payload.descriptionBn)
        changed.push("descriptionBn");
    if (existing.imageUrl !== payload.imageUrl)
        changed.push("imageUrl");
    if (existing.iconUrl !== payload.iconUrl)
        changed.push("iconUrl");
    if (existing.imagePath !== payload.imagePath)
        changed.push("imagePath");
    if (existing.thumbPath !== payload.thumbPath)
        changed.push("thumbPath");
    if (existing.slug !== payload.slug)
        changed.push("slug");
    if ((existing.parentId ?? "") !== (payload.parentId ?? ""))
        changed.push("parentId");
    if (existing.groupId !== nextGroupId)
        changed.push("groupId");
    if (existing.isFeatured !== payload.isFeatured)
        changed.push("isFeatured");
    if (existing.showOnHome !== payload.showOnHome)
        changed.push("showOnHome");
    if (existing.isActive !== payload.isActive)
        changed.push("isActive");
    if (existing.sortOrder !== payload.sortOrder)
        changed.push("sortOrder");
    return changed;
}
async function ensureParentIsValid(tx, categoryId, parentId) {
    if (!parentId)
        return;
    ...
    while (cursor) {
        if (visited.has(cursor)) {
            throw new https_1.HttpsError("failed-precondition", "Detected a circular category parent chain.");
        }
        visited.add(cursor);
        if (cursor === categoryId) {
            throw new https_1.HttpsError("failed-precondition", "This parent selection would create a circular hierarchy.");
        }
    ...
    try {
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (changedFields.length === 0) {
                throw new https_1.HttpsError("failed-precondition", "No category changes were detected.");
            }
            tx.set(categoryRef, buildUpdatedCategoryMap(categoryId, payload, existing), { merge: true });
            ...
            tx.set(logRef, (0, audit_log_core_1.buildAdminAuditLogDoc)(logRef.id, actor, {
                ...
                afterData: buildAuditCategoryState(categoryId, payload, existing.productsCount),
                metadata: {
                    changedFields,
            ...
            if (existing.imagePath.length > 0 &&
