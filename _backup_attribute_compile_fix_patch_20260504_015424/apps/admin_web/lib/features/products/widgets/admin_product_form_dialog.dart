...
import 'product_variation_dialog_page.dart';
...
class _AdminProductFormDialogState extends State<AdminProductFormDialog> {
  ...
  MBProductVariation? get _explicitDefaultVariation {
    final defaults = _variations
        .where((item) => item.isEnabled && item.isDefault)
        .toList()
  ...
  String? get _effectiveThumbnailUrl {
    if (_isVariableProduct) {
      final url =
          _explicitDefaultVariation?.effectiveThumbImageUrl.trim() ??
  ...
  List<String> get _effectiveImageUrls {
    // Variable products must not leak variation images into root imageUrls.
    // Root imageUrls are product-level gallery images only.
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (!_isVariableProduct) ...[
                          KeyedSubtree(
                            key: _mediaKey,
                            child: _buildProductImageCardDesignSection(context),
                          ),
                          const SizedBox(height: 16),
                        ],
                        ...
                        KeyedSubtree(
                          key: _cardStyleKey,
                          child: _buildGalleryImagesSection(context),
                        ),
                        const SizedBox(height: 16),
      ...
      child: Column(
        ...
        children: [
          ...
          _buildModeGuideRow(
            ...
            description:
            'Basic info, category/brand, pricing, inventory, quantity, visibility, purchase options, product image/card design, gallery images, attributes, variations, and audit fields.',
          ),
          const SizedBox(height: 12),
          _buildModeGuideRow(
            context,
  ...
  MBProductMedia? get _primaryProductImageMedia {
    final primaryItems = _mediaItems
        .where(
          (item) =>
              item.isEnabled &&
              item.type == 'image' &&
    ...
    if (primaryItems.isNotEmpty) return primaryItems.first;

    if (_isVariableProduct) return null;

    final imageItems = _mediaItems
        .where((item) => item.isEnabled && item.type == 'image')
        .toList()
      ..sort((a, b) => a.sortOrder.compareTo(b.sortOrder));

    return imageItems.isEmpty ? null : imageItems.first;
  }

  List<MBProductMedia> get _galleryMediaItems {
    final primaryId = _primaryProductImageMedia?.id;
    final gallery = _mediaItems.where((item) {
      if (!item.isEnabled || item.type != 'image') return false;
      if (_isVariableProduct) return true;
      if (primaryId != null && item.id == primaryId) return false;
      return !item.isPrimary && item.role.trim().toLowerCase() != 'thumbnail';
    }).toList()
      ..sort((a, b) => a.sortOrder.compareTo(b.sortOrder));
  ...
  int get _maxProductGalleryImageItems => _isVariableProduct ? 2 : 9;

  Widget _buildProductImageSurface(BuildContext context, MBProductMedia? item) {
    final pendingBytes = item?.pendingFullBytes ??
        item?.pendingCardBytes ??
    ...
    if (pendingBytes != null && pendingBytes.isNotEmpty) {
      ...
      final url = item?.effectiveFullUrl.trim().isNotEmpty == true
          ? item!.effectiveFullUrl.trim()
          : (item?.effectiveCardUrl.trim().isNotEmpty == true
              ? item!.effectiveCardUrl.trim()
              : (item?.effectiveThumbUrl.trim() ?? ''));
  ...
  Widget _buildProductImageCardActionBox(
    BuildContext context, {
    required String title,
  ...
  Widget _buildProductImageCardDesignSection(BuildContext context) {
    final primaryImage = _primaryProductImageMedia;
    final previewProduct = _buildProductFromForm();

    return SectionCard(
      title: 'Product Image & Card Design',
      subtitle:
          'Primary product image and real customer-app card design. Hidden for variable products.',
      child: LayoutBuilder(
        builder: (context, constraints) {
          final stack = constraints.maxWidth < 760;
          final imageBox = _buildProductImageCardActionBox(
            context,
            title: 'Primary Product Image',
            subtitle:
                'One primary image. Generates original, full, card, thumb, and tiny sizes.',
            preview: _buildProductImageSurface(context, primaryImage),
            action: FilledButton.icon(
              onPressed: _selectPrimaryProductImage,
              icon: const Icon(Icons.image_outlined),
              label: const Text('Select Image'),
            ),
          );
          final cardBox = _buildProductImageCardActionBox(
            context,
            title: 'Card Preview',
            subtitle:
                'Uses the same real renderer path as saved Studio V3 designs.',
            preview: _buildRealCardPreviewSurface(
              context,
  ...
  Widget _buildGalleryImagesSection(BuildContext context) {
    final galleryItems = _galleryMediaItems;
    final maxItems = _maxProductGalleryImageItems;
    final subtitle = _isVariableProduct
        ? 'Variable product general gallery. Add up to 2 images. These save original and full image only.'
        : 'Product detail gallery. Add up to 9 gallery images. These save original and full image only.';

    return SectionCard(
      title: 'Gallery Images',
      subtitle: subtitle,
      action: FilledButton.icon(
        onPressed: galleryItems.length >= maxItems ? null : _addGalleryMediaItem,
        icon: const Icon(Icons.add_photo_alternate_outlined),
        label: const Text('Add Gallery Image'),
      ),
      child: galleryItems.isEmpty
          ? EmptyBlock(
              message: _isVariableProduct
                  ? 'No variable-product general gallery images added yet. Add up to 2 images.'
                  : 'No product gallery images added yet. Add up to 9 images for the product detail gallery.',
            )
          : Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            SizedBox(
                              height: 130,
                              width: double.infinity,
                              child: Center(child: _buildMediaItemPreview(item)),
                            ),
                            const SizedBox(height: 8),
                            Text(
                              item.labelEn.trim().isEmpty
                                  ? (item.effectiveFullUrl.trim().isEmpty
                                      ? item.id
                                      : item.effectiveFullUrl.trim())
                                  : item.labelEn.trim(),
                              maxLines: 1,
                            ...
                            const SizedBox(height: 4),
                            Text(
                              'role: gallery | order: ${item.sortOrder}${item.hasPendingUpload ? ' | pending upload' : ''}',
                              maxLines: 2,
                              overflow: TextOverflow.ellipsis,
                              style: Theme.of(context).textTheme.bodySmall,
                            ),
                            ...
                            Row(
                              ...
                              children: [
                                IconButton(
                                  tooltip: 'Edit gallery image',
                                  onPressed: () => _editMediaItem(item),
                                  icon: const Icon(Icons.edit_outlined),
                                ),
                                IconButton(
                                  tooltip: 'Delete gallery image',
                                  onPressed: () {
                                    setState(() {
                ...
                const SizedBox(height: 12),
                Text(
                  '${galleryItems.length} / $maxItems gallery images used.',
                  style: Theme.of(context).textTheme.bodySmall,
                ),
              ],
            ),
  ...
  Widget _buildRealCardPreviewSurface(
    BuildContext context, {
  ...
  List<String> _variationAttributeLines(MBProductVariation variation) {
    ...
    for (final attribute in _variationAttributesSnapshot) {
      ...
      final matchedValue =
          attribute.values.cast<MBProductAttributeValue?>().firstWhere(
  ...
  String _variationPreviewFullUrl(MBProductVariation variation) {
    final pending = _pendingVariationImagesById[variation.id];
    if (pending != null) {
      final value = _dataImageUrl(pending.fullBytes);
      if (value.isNotEmpty) return value;
    }
    return variation.effectiveFullImageUrl.trim();
  }

  String _variationPreviewCardUrl(MBProductVariation variation) {
    final pending = _pendingVariationImagesById[variation.id];
    if (pending != null) {
  ...
  String _variationPreviewThumbUrl(MBProductVariation variation) {
    final pending = _pendingVariationImagesById[variation.id];
    if (pending != null) {
      final value = _dataImageUrl(pending.thumbBytes ?? pending.cardBytes);
      if (value.isNotEmpty) return value;
    }
  ...
  MBProduct _buildVariationPreviewProduct(MBProductVariation variation) {
    ...
    final variationCardDesignJson = _normalizedCardDesignJsonForSave(variation.cardDesignJson);
    final productCardDesignJson = _normalizedCardDesignJsonForSave(_cardDesignJson);
    final resolvedCardDesignJson =
        variationCardDesignJson != null && variationCardDesignJson.isNotEmpty
            ? variationCardDesignJson
            : (productCardDesignJson != null && productCardDesignJson.isNotEmpty
                ? productCardDesignJson
                : null);

    return base.copyWith(
      productType: 'simple',
      titleEn: variation.titleEn.trim().isEmpty
          ? base.titleEn
          : variation.titleEn.trim(),
      titleBn: variation.titleBn.trim().isEmpty
          ? base.titleBn
          : variation.titleBn.trim(),
      shortDescriptionEn: variation.shortDescriptionEn.trim().isEmpty
          ? base.shortDescriptionEn
          : variation.shortDescriptionEn.trim(),
      shortDescriptionBn: variation.shortDescriptionBn.trim().isEmpty
          ? base.shortDescriptionBn
          : variation.shortDescriptionBn.trim(),
      descriptionEn: variation.descriptionEn.trim().isEmpty
          ? base.descriptionEn
          : variation.descriptionEn.trim(),
      descriptionBn: variation.descriptionBn.trim().isEmpty
          ? base.descriptionBn
          : variation.descriptionBn.trim(),
      thumbnailUrl: thumbUrl.isNotEmpty ? thumbUrl : fullUrl,
      imageUrls: fullUrl.isEmpty ? const <String>[] : <String>[fullUrl],
      mediaItems: fullUrl.isEmpty
          ? const <MBProductMedia>[]
          : <MBProductMedia>[variationMedia],
      price: variation.price,
      salePrice: variation.salePrice,
      clearSalePrice: variation.salePrice == null,
      costPrice: variation.costPrice,
      clearCostPrice: variation.costPrice == null,
      stockQty: variation.stockQty,
      inventoryMode: variation.inventoryMode,
      trackInventory: variation.trackInventory,
      supportsInstantOrder: variation.supportsInstantOrder,
      supportsScheduledOrder: variation.supportsScheduledOrder,
      allowBackorder: variation.allowBackorder,
      quantityType: variation.quantityType,
      quantityValue: variation.quantityValue,
      toleranceType: variation.toleranceType,
      tolerance: variation.tolerance,
      isToleranceActive: variation.isToleranceActive,
      deliveryShift: variation.deliveryShift,
      unitLabelEn: variation.unitLabelEn,
      clearUnitLabelEn:
          variation.unitLabelEn == null || variation.unitLabelEn!.trim().isEmpty,
      unitLabelBn: variation.unitLabelBn,
      clearUnitLabelBn:
          variation.unitLabelBn == null || variation.unitLabelBn!.trim().isEmpty,
      ...
      cardConfig: variation.cardConfig ?? base.cardConfig,
      cardDesignJson: resolvedCardDesignJson,
      clearCardDesignJson: resolvedCardDesignJson == null,
    );
  }

  Widget _buildVariationReadonlyImagePreview(
    BuildContext context,
  ...
  Widget _buildVariationInfoLine(
    BuildContext context,
    String label,
    String value,
  ) {
  ...
  Widget _buildVariationInfoBlock(
    BuildContext context,
    MBProductVariation item,
    List<String> attributeLines,
  ) {
    final salePrice = item.salePrice == null
        ? '-'
        : item.salePrice!.toStringAsFixed(2);
    final barcode = (item.barcode ?? '').trim();
    final cardDesignStatus = item.cardDesignJson?.trim().isNotEmpty == true
        ? 'own variation design'
        : (_hasCardDesignJson ? 'product design fallback' : 'default renderer');

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
          item.titleEn.trim().isEmpty ? 'Untitled Variation' : item.titleEn.trim(),
          maxLines: 2,
          overflow: TextOverflow.ellipsis,
          style: Theme.of(context).textTheme.titleSmall?.copyWith(
                fontWeight: FontWeight.w900,
        ...
        const SizedBox(height: 10),
        _buildVariationInfoLine(context, 'Title BN', item.titleBn),
        _buildVariationInfoLine(context, 'SKU', item.sku),
        _buildVariationInfoLine(context, 'Barcode', barcode),
        _buildVariationInfoLine(context, 'Price', item.price.toStringAsFixed(2)),
        _buildVariationInfoLine(context, 'Sale', salePrice),
        _buildVariationInfoLine(context, 'Stock', item.stockQty.toString()),
        _buildVariationInfoLine(context, 'Status', item.status),
        _buildVariationInfoLine(context, 'Default', item.isDefault.toString()),
        _buildVariationInfoLine(context, 'Enabled', item.isEnabled.toString()),
        _buildVariationInfoLine(context, 'Card', cardDesignStatus),
        if (attributeLines.isNotEmpty) ...[
          const SizedBox(height: 4),
          ...attributeLines.map(
            (line) => _buildVariationInfoLine(context, 'Attribute', line),
          ),
        ],
      ],
    );
  ...
  Widget _buildVariationRowActions(BuildContext context, MBProductVariation item) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        IconButton(
  ...
  Widget _buildVariationPreviewRow(
    BuildContext context,
    MBProductVariation item,
  ) {
    final attributeLines = _variationAttributeLines(item);
    final previewProduct = _buildVariationPreviewProduct(item);
  ...
  Widget _buildVariationsSection(BuildContext context) {
    ...
    return SectionCard(
      title: 'Variations',
      subtitle:
          'One row per variation. Each row is a read-only preview of variation info, image, and real card design. Edit image/card design inside the variation dialog.',
      action: FilledButton.icon(
      ...
      child: items.isEmpty
          ...
          : Column(
              children: items
                  .map((item) => _buildVariationPreviewRow(context, item))
                  .toList(growable: false),
            ),
    );
  }
    ...
    try {
      ...
      if (decoded is Map) {
        ...
        if (templateId.isNotEmpty) {
          return _normalizeAdvancedCardTemplateId(templateId);
        }
        final layout = decoded['layout'];
        if (layout is Map) {
          final layoutType = (layout['cardLayoutType'] ?? '').toString().trim();
          if (layoutType.isNotEmpty) {
            return _normalizeAdvancedCardTemplateId(layoutType);
          }
        }
      }
    } catch (_) {
      // Keep save safe by falling back below.
    }
  ...
  String _normalizeCardLayoutTypeForSave(
    ...
    for (final variant in MBCardVariant.values) {
      if (variant.id.toLowerCase() == advanced.toLowerCase()) {
        return variant.id;
      }
    }
  ...
  String _normalizeAdminCardVariantId(String raw) {
    ...
    final advanced = _normalizeAdvancedCardTemplateId(normalized);
    if (advanced == 'hero_poster_circle_diagonal_v1') {
      return advanced;
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            _hasCardDesignJson
                ? 'Use Card Design to edit the saved product-card design.'
  ...
  Widget _buildCardStyleSection(BuildContext context) {
    ...
    return SectionCard(
      title: 'Product Image & Card Design',
      subtitle:
          'Use Card Design to select, design, preview, and save the product-card layout.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
...
Wrap(
  ...
  MBPreparedImageSet _preparedImageSetFromPendingMedia(MBProductMedia item) {
    ...
    // Gallery images are full-only, so their pending original/card/thumb/tiny
    // bytes may be intentionally empty. The upload helper still expects a
    // complete MBPreparedImageSet object, therefore fullBytes is used as a safe
    // in-memory fallback. Gallery upload calls pass uploadOriginalCardTiny=false,
    ...
    return MBPreparedImageSet(
      ...
      croppedByteLength: item.cropSizeBytes ?? cardBytes.lengthInBytes,
  ...
  String _variationMediaFileStemFor(
    ...
    final base = prepared.baseName.trim();
    return base.isEmpty ? variation.id : base;
  }
  ...
  MBProductVariation? _firstVariationBySortOrder() {
    ...
    return enabled.isNotEmpty ? enabled.first : items.first;
  }

  void _setSingleDefaultVariation(String variationId) {
    _variations = _variations
        .map(
          (item) => item.copyWith(
            isDefault: item.id == variationId,
          ),
        )
        .toList()
      ..sort((a, b) => a.sortOrder.compareTo(b.sortOrder));
  ...
  Future<bool> _ensureVariableDefaultVariationForSave() async {
    if (!_isVariableProduct) return true;

    if (_variations.isEmpty) {
      await _showDialogPrompt(
        'Default Variation Required',
        'Add at least one variation before saving this variable product.',
      );
      return false;
    }
    ...
    if (defaults.length == 1) return true;

    if (defaults.length > 1) {
      setState(() => _setSingleDefaultVariation(defaults.first.id));
      return true;
    }

    final candidate = _firstVariationBySortOrder();
    if (candidate == null) return false;

    final title = candidate.titleEn.trim().isEmpty
        ? candidate.id
    ...
    final accept = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Default Variation Required'),
        content: Text(
          'No default variation is selected. Set "$title" as the default variation now?',
        ),
        actions: [
          TextButton(
    ...
    if (accept != true) {
      await _showDialogPrompt(
        'Default Variation Required',
        'Please edit a variation and mark it as default before saving.',
      );
      return false;
    }

    setState(() => _setSingleDefaultVariation(candidate.id));
    return true;
  }
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      await _uploadPendingVariationImages();
    } catch (error) {
      if (!mounted) return;
      await _showProductMediaPrompt(
        'Image Upload Failed',
  ...
  MBProduct _buildProductFromForm() {
    ...
    final rootCardVariation = _isVariableProduct
        ? (_explicitDefaultVariation ?? _primaryVariationForOwnership)
        : null;
    final selectedCardConfig =
        rootCardVariation?.effectiveCardConfig ?? _selectedCardInstanceConfig;
    final variationDesignJson =
        _normalizedCardDesignJsonForSave(rootCardVariation?.cardDesignJson);
    final productDesignJson = _normalizedCardDesignJsonForSave(_cardDesignJson);
    final rootCardDesignJson = _isVariableProduct
        ? (variationDesignJson != null && variationDesignJson.isNotEmpty
            ? variationDesignJson
            : null)
        : (productDesignJson != null && productDesignJson.isNotEmpty
            ? productDesignJson
            : null);
    final rootCardLayoutType = _normalizeCardLayoutTypeForSave(
      (rootCardVariation?.cardLayoutType ?? '').trim().isNotEmpty
          ? rootCardVariation!.cardLayoutType!.trim()
          : selectedCardConfig.variantId,
    ...
    return (_source.id.trim().isEmpty ? MBProduct.empty() : _source).copyWith(
      ...
      purchaseOptions: [..._purchaseOptions]
        ..sort((a, b) => a.sortOrder.compareTo(b.sortOrder)),
      cardLayoutType: rootCardLayoutType,
  ...
  void _normalizeProductMediaPrimary() {
    ...
    for (var i = 0; i < _mediaItems.length; i++) {
      ...
      _mediaItems[i] = item.copyWith(
        ...
        role: shouldBePrimary ? 'thumbnail' : 'gallery',
        type: 'image',
        sortOrder: i,
      );
  ...
  Future<void> _selectPrimaryProductImage() async {
    if (_isVariableProduct) {
      await _showProductMediaPrompt(
        'Variation-owned Image',
        'Variable products do not use the product-level primary image. Use the image section inside each variation.',
      );
      return;
    }

    final existing = _primaryProductImageMedia;
    ...
    setState(() {
      if (existing == null) {
        _mediaItems.insert(0, normalized);
      } else {
        final index = _mediaItems.indexWhere((element) => element.id == existing.id);
        if (index == -1) {
          _mediaItems.insert(0, normalized);
        } else {
          _mediaItems[index] = normalized;
        }
      }
      _normalizeProductMediaPrimary();
    });
  ...
  Future<void> _addGalleryMediaItem() async {
    final maxItems = _maxProductGalleryImageItems;
    if (_galleryMediaItems.length >= maxItems) {
      await _showProductMediaPrompt(
        'Gallery Limit Reached',
        _isVariableProduct
            ? 'A variable product can have up to 2 general gallery images.'
            : 'A non-variable product can have up to 9 gallery images plus 1 primary image.',
      );
      return;
    }

    final sortOrder = _mediaItems.length;
    final result = await showDialog<MBProductMedia>(
      context: context,
      builder: (_) => MediaItemDialog(
        ...
        maxItems: _isVariableProduct ? 2 : _maxProductMediaItems,
        currentItemCount: _isVariableProduct ? _galleryMediaItems.length : _mediaItems.length,
        useProductPortraitPreset: true,
        forceImageOnly: true,
        galleryFullOnly: true,
      ),
    );
  ...
  Future<void> _addMediaItem() async {
    if (_isVariableProduct) {
      await _addGalleryMediaItem();
      return;
    }

    if (_primaryProductImageMedia == null) {
      await _selectPrimaryProductImage();
      return;
    }
  ...
  Future<void> _editMediaItem(MBProductMedia item) async {
    final result = await showDialog<MBProductMedia>(
      ...
      builder: (_) => MediaItemDialog(
        ...
        useProductPortraitPreset: true,
        forceImageOnly: true,
        galleryFullOnly: _isVariableProduct || item.id != _primaryProductImageMedia?.id,
      ),
    );
    ...
    setState(() {
      ...
      if (index != -1) {
        final isPrimaryImage = !_isVariableProduct && item.id == _primaryProductImageMedia?.id;
        _mediaItems[index] = result.copyWith(
          isPrimary: isPrimaryImage,
          role: isPrimaryImage ? 'thumbnail' : 'gallery',
          type: 'image',
        );
        _normalizeProductMediaPrimary();
      }
  ...
  MBProductVariation _normalizeVariationForSave(MBProductVariation variation) {
    final normalizedDesignJson = _normalizedCardDesignJsonForSave(variation.cardDesignJson);
    final normalizedLayoutType = _normalizeCardLayoutTypeForSave(
      variation.cardLayoutType ?? '',
      cardDesignJson: normalizedDesignJson,
    );

    return variation.copyWith(
      cardLayoutType: normalizedLayoutType,
      cardDesignJson: normalizedDesignJson,
      clearCardDesignJson: normalizedDesignJson == null || normalizedDesignJson.isEmpty,
    );
  }
  ...
  Future<void> _addVariation() async {
    ...
    final result = await showDialog<ProductVariationDialogResult>(
      context: context,
    ...
    final variation = _normalizeVariationForSave(result.variation);
    final duplicateMessage = _findDuplicateVariationMessage(variation);
    if (duplicateMessage != null) {
