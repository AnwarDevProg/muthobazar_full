...
//
// Patch 3 adds keyboard move/resize/delete and stronger canvas focus.
...
class _CanvasKeyboardScope extends StatefulWidget {
  const _CanvasKeyboardScope({
    required this.document,
    required this.onMoveNode,
    required this.onDeleteNode,
    required this.child,
  ...
  @override
  State<_CanvasKeyboardScope> createState() => _CanvasKeyboardScopeState();
}

class _CanvasKeyboardScopeState extends State<_CanvasKeyboardScope> {
  ...
  void _handleKeyEvent(KeyEvent event) {
    ...
    if (isResize) {
      _resizeNode(selectedNode, key, isFast: isFast);
    } else {
      _moveNode(selectedNode, key, isFast: isFast);
    }
  }
  ...
  void _moveNode(
    ...
    final pixelStep = isFast ? 10.0 : 2.0;
    final xStep = pixelStep / widget.document.cardWidth;
    final yStep = pixelStep / widget.document.cardHeight;

    var nextX = node.position.x;
    var nextY = node.position.y;

    if (key == LogicalKeyboardKey.arrowLeft) nextX -= xStep;
    if (key == LogicalKeyboardKey.arrowRight) nextX += xStep;
    if (key == LogicalKeyboardKey.arrowUp) nextY -= yStep;
    if (key == LogicalKeyboardKey.arrowDown) nextY += yStep;

    final halfWidth = (node.size.width / widget.document.cardWidth) / 2;
    final halfHeight = (node.size.height / widget.document.cardHeight) / 2;

    widget.onMoveNode(
      node.copyWith(
        position: node.position.copyWith(
          x: _clampDouble(nextX, halfWidth, 1.0 - halfWidth),
          y: _clampDouble(nextY, halfHeight, 1.0 - halfHeight),
        ),
      ),
    );
  }
  ...
  void _resizeNode(
    ...
    final step = isFast ? 10.0 : 2.0;
    var nextWidth = node.size.width;
    var nextHeight = node.size.height;

    if (key == LogicalKeyboardKey.arrowLeft) nextWidth -= step;
    if (key == LogicalKeyboardKey.arrowRight) nextWidth += step;
    if (key == LogicalKeyboardKey.arrowUp) nextHeight -= step;
    if (key == LogicalKeyboardKey.arrowDown) nextHeight += step;

    final maxWidth = math.min(node.size.maxWidth, widget.document.cardWidth);
    final maxHeight = math.min(node.size.maxHeight, widget.document.cardHeight);

    final safeWidth = _clampDouble(
      nextWidth,
      math.max(4.0, node.size.minWidth),
      math.max(node.size.minWidth, maxWidth),
    );
    final safeHeight = _clampDouble(
      nextHeight,
      math.max(4.0, node.size.minHeight),
      math.max(node.size.minHeight, maxHeight),
    );
    ...
    return Container(
      ...
      child: const Row(
        children: <Widget>[
          ...
          Text(
            'Drag · Click · Arrow move · Ctrl+Arrow resize · Delete',
            style: TextStyle(
              color: Color(0xFF747B8A),
              fontSize: 11,
              fontWeight: FontWeight.w700,
...
class _ShapeNode extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color = _hexColor(node.style['backgroundHex'], Colors.white)
        .withValues(
          alpha: _asDouble(node.style['opacity'], 1.0)
              .clamp(0.0, 1.0)
              .toDouble(),
        );
...
String _resolveNodeText(dynamic product, MBAdvancedDesignNode node) {
  ...
  if (node.elementType == 'price' || node.elementType == 'mrp') {
